...
def test_search_books(client):    
    ...
    data = get_url(client, f"xxx/books", check_empty=False)
    assert len(data.results) == 0
    
    
    
def test_get_verse(client):
    data = get_url(client, f"{MG_VERSION}/verses/66/15?from_verse=1&to_chapter=17&to_verse=3")    
    ...
    data = get_url(client, f"{MG_VERSION}/verses/19/91?from_verse=2&to_chapter=94&to_verse=3")
    assert len(data.results) == 38
    
    # across different books
    data = get_url(client, f"{MG_VERSION}/verses/40/28?from_verse=1&to_book=41&to_chapter=2&to_verse=5")
    ...
    data = get_url(client, f"{MG_VERSION}/verses/40/1?from_verse=1&to_book=41&to_chapter=2&to_verse=5")    
    ...
    assert data.count == 7958
    data = get_url(client, f"{MG_VERSION}/verses/40/1?from_verse=1&to_book=66&offset=100&max_results=25")           
